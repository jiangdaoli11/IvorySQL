...
select PGEXTRACT(epoch from '256 microseconds'::interval * (2^55)::float8);
...
--
SELECT f1,
    PGEXTRACT(MICROSECOND FROM f1) AS MICROSECOND,
    PGEXTRACT(MILLISECOND FROM f1) AS MILLISECOND,
    PGEXTRACT(SECOND FROM f1) AS SECOND,
    PGEXTRACT(MINUTE FROM f1) AS MINUTE,
    PGEXTRACT(HOUR FROM f1) AS HOUR,
    PGEXTRACT(DAY FROM f1) AS DAY,
    PGEXTRACT(MONTH FROM f1) AS MONTH,
    PGEXTRACT(QUARTER FROM f1) AS QUARTER,
    PGEXTRACT(YEAR FROM f1) AS YEAR,
    PGEXTRACT(DECADE FROM f1) AS DECADE,
    PGEXTRACT(CENTURY FROM f1) AS CENTURY,
    PGEXTRACT(MILLENNIUM FROM f1) AS MILLENNIUM,
    PGEXTRACT(EPOCH FROM f1) AS EPOCH
    FROM INTERVAL_TBL;

SELECT PGEXTRACT(FORTNIGHT FROM INTERVAL '2 days');  -- error
SELECT PGEXTRACT(TIMEZONE FROM INTERVAL '2 days');  -- error

SELECT PGEXTRACT(DECADE FROM INTERVAL '100 y');
SELECT PGEXTRACT(DECADE FROM INTERVAL '99 y');
SELECT PGEXTRACT(DECADE FROM INTERVAL '-99 y');
SELECT PGEXTRACT(DECADE FROM INTERVAL '-100 y');

SELECT PGEXTRACT(CENTURY FROM INTERVAL '100 y');
SELECT PGEXTRACT(CENTURY FROM INTERVAL '99 y');
SELECT PGEXTRACT(CENTURY FROM INTERVAL '-99 y');
SELECT PGEXTRACT(CENTURY FROM INTERVAL '-100 y');
